...
var objCajas1= new THREE.Object3D();
...
const loaderCajas1= new OBJLoader(manager);
var mtlCajas = new MTLLoader(manager);
var mtlCajas1 = new MTLLoader(manager);
...
cube1.position.set(5, 5, 9);
cube1.visible = false;
scene.add(cube1);

const cb_Vida = new THREE.Box3();
cb_Vida.setFromObject(cube1);
...
const cube2Material = new THREE.MeshPhongMaterial({ color: "aqua" });
const cube2 = new THREE.Mesh(cube2Geometry, cube2Material);
cube2.position.set(8, 5, 5);
cube2.visible = false;

scene.add(cube2);

const cb_Player = new THREE.Box3();
cb_Player.setFromObject(cube2);
...
mtlCajas1.load('./Modelos/cajitas.mtl', function (materials){
    materials.preload();
    loaderCajas1.setMaterials(materials);
    loaderCajas1.load('./Modelos/cajitas.obj',
       function(object){
        object.name="Cajas1";
        object.scale.copy(new THREE.Vector3(5,5,5));
        objCajas1 = object; // Corrección aquí
...
function checkCollitions() {
	...
	// console.log(objCinna.position)
	if (cb_Vida.containsBox(cb_Player)) {
		cube2.material.color = new THREE.Color("red");
	...
	if (cb_Vida.intersectsBox(cb_Player)) {
	  console.log("xdd")
	  if(vidallena == 0){
			
		eliminarCorazon(objItemCorazon);
		
	// Si hay colisión, eliminar u ocultar el corazón
	

	// Incrementar el contador de vidas y actualizar la imagen
	if (vida > 0 && vida < 5) {
		vida++;
		actualizarImagenVida(vida);
		 // Desactivar la colisión del corazón
		 
		 
	}

	if (objItemCorazon) {
		objItemCorazon.scale.set(0, 0, 0);
	}

	// Si la vida es cero, restablecer a 1
	if (vida == 5) {
		vidallena = 1;
	}
...
function checkCinnamorollCajasCollision1() {
    if (checkCollision(objCinna, objCajas1)) {
        // Si hay colisión, retroceder Cinnamoroll a su posición anterior
        objCinna.position.copy(previousCinnaPosition);
...
function eliminarCorazon(corazon) {
    // Supongamos que estamos usando Three.js
    scene.remove(corazon); // Esto eliminará el corazón de la escena
    corazon.isCollidable = false; // Desactivar la colisión del corazón
}

/////////////////////////////////ACTUALIZAR VIDA CINNA
function actualizarImagenVida(vida) {
    switch (vida) {
        case 5:
            document.getElementById('vidasImage').src = './Imagenes/vidas/vidas5.png'; // Imagen de vida completa
			vidallena = 1;
            break;
        case 4:
...
function checkCinnamorollHongo2Collision() {
	...
	if (checkCollision17(objCinna, objHongos2)) {
        ...
        if (!speedBoostActive && currentSpeed === originalSpeed && doOnce1 === 0) {
            currentSpeed = 2;
...
function checkCinnamorollHongo3Collision() {
    if (checkCollision18(objCinna, objHongos3)) {
        ...
        // Aumentar la velocidad temporalmente si no está activo
        if (!speedBoostActive && currentSpeed === originalSpeed && doOnce === 0) {
            currentSpeed = 2;
            speedBoostActive = true;
			doOnce = 1;
            // Restaurar la velocidad después de 5 segundos
...
function checkPurinKirbyCollision() {
    if (checkCollision20(objCinna, objKirby2)) {
        // Si hay colisión, retroceder Cinnamoroll a su posición anterior
        objCinna.position.copy(previousPompompurinPosition20);
        ...
        // Si no hay colisión, actualizar la posición anterior de Cinnamoroll
        previousPompompurinPosition20.copy(objCinna.position);
    }
}
...
function reiniciar() {
    ...
    window.location.href = "Nivel1.html"; // Redirige a nivel1.html al reiniciar
}
...
btnContinuar.addEventListener("click", function() {
    window.location.href = "Nivel2.html"; // Redirige a Nivel2.html al hacer clic en Continuar
});
...
document.getElementById("Continuar").addEventListener("click", function() {
	...
	localStorage.setItem('tiempoJugador2', `${displayMinutes}:${displaySeconds}`); // Tiempo del Jugador 2
	localStorage.setItem('tiempoJugador1',  `${displayMinutes}:${displaySeconds}`);
...
function animate() {
	...
	cb_Vida.copy(cube1.geometry.boundingBox).applyMatrix4(cube1.matrixWorld);

	cb_Player.copy(cube2.geometry.boundingBox).applyMatrix4(cube2.matrixWorld);
	...
	checkPurinNota1Collision();
	 ...
	 checkPurinNota2Collision();
	  ...
	  checkPurinNota3Collision();
	   ...
	   checkCinnamorollCajasCollision1();
...
document.addEventListener('keydown', (event) => {
    ...
    const moveAmount =  currentSpeed; // Ajusta la velocidad aquí
    ...
    switch (key) {
        ...
        case 'arrowup':
            ...
            cube2.position.z = objCinna.position.z;
        ...
        case 'arrowleft':
			...
			cube2.position.x = objCinna.position.x;
		...
		case 'm':
			...
			increaseScore(100); //100 PUNTOS POR NOTA
		...
		case 'e':
			if (interaccionNota1P) {
				...
				increaseScore2(100); // 100 PUNTOS POR NOTA
